    ...
    foreach ($mb in $mailboxes) {
        ...
        if ($mb.SendAsUser -and $mb.SendAsUser.Trim() -ne "") {
            ...
            foreach ($user in $sendAsUsers) {
                ...
                if (-not $WhatIf) {
                    try {
                        Add-RecipientPermission $mb.SharedMailboxEmail -AccessRights SendAs -Trustee $user -Confirm:$false -ErrorAction Stop
        ...
        if ($mb.FullAccessUser -and $mb.FullAccessUser.Trim() -ne "") {
            ...
            foreach ($user in $fullAccessUsers) {
                ...
                if (-not $WhatIf) {
                    try {
                        Add-MailboxPermission $mb.SharedMailboxEmail -User $user -AccessRights FullAccess -AutoMapping $true -Confirm:$false -ErrorAction Stop
